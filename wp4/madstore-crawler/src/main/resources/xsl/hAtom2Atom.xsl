    ...
    <!--
        hAtom2Atom.xsl An XSLT stylesheet for transforming hAtom documents into Atom documents. (last updated: 2007-08-09) SUPPORTED XSLT ENGINES 4xslt <http://4suite.org/> libxslt <http://xmlsoft.org/XSLT/> Saxon <http://saxon.sourceforge.net/> Xalan-C <http://xml.apache.org/xalan-c/> Xalan-J <http://xml.apache.org/xalan-j/> USAGE INSTRUCTIONS It is highly recommended that you set the stylesheet's source-uri parameter to the source URI of your input document. Your XHTML document must have the
        namespace "http://www.w3.org/1999/xhtml". If it does not or if your input document is written in HTML, filter it through "tidy -asxhtml" <http://tidy.sourceforge.net/> before processing it with hAtom2Atom.xsl. STYLESHEET PARAMETERS $source-uri: Source URI of the input document, e.g: http://example.com/foo.html It is highly recommended that you set this parameter. $content-type: The content type of your input document. The default is "text/html". $implicit-feed: If no feeds are found, the
        value of $implicit-feed determines wether the whole document should be treated as feed or if the first hentry should be extracted as stand-alone atom:entry. The default is "1". $debug-comments: If $debug-comments is set to "1", hAtom2Atom.xsl will add comments which can aid with debugging. The default is "1". $sanitize-html: If $sanitize-html is set to "1", hAtom2Atom.xsl will remove all attributes and elements which are not listed at <http://feedparser.org/docs/html-sanitization.html>
        from atom:summary and atom:content. The default is "1". $tag-schemes: If $tag-schemes is set to "1", hAtom2Atom.xsl will add the "scheme" attribute to <atom:category> when extracting rel-tags. The value of scheme will be the tag space of the extracted rel-tag. The default is "1". NOTES Code sections which are extensions to the current hAtom draft are enclosed by "[extension]" and "[/extension]". For various reasons we need to use the node-set() extension function, see
        <http://www.xml.com/pub/a/2003/07/16/nodeset.html> for an introduction. This stylesheet and the hAtom specification are still works in progress. Use it at your own risk! SEE ALSO For the latest version of this stylesheet: <http://rbach.priv.at/Microformats/hAtom2Atom/> Mercurial repository: <http://hg.microformats.org/x2v/> Information about hAtom: <http://microformats.org/wiki/hatom> Information about Atom: <http://www.ietf.org/rfc/rfc4287> LICENSE Copyright 2005 Luke Arno
        <http://lukearno.com/> Copyright 2005-07 Robert Bachmann <http://rbach.priv.at/> Copyright 2005-06 Benjamin Carlyle <http://soundadvice.id.au/> This work is licensed under The W3C Open Source License http://www.w3.org/Consortium/Legal/copyright-software-19980720 ACKNOWLEDGEMENTS Structure of the multi-valued attribute selection trick and templates for datetime to UTC conversion taken from Brian Suda's X2V: <http://suda.co.uk/projects/X2V/> The list of acceptable HTML elements and
        attributes was taken from Mark Pilgrim's Universal Feed Parser: <http://feedparser.org/> This work is based on hAtom2Atom.xsl version 0.0.6 from <http://lukearno.com/projects/hAtom/>
    -->


<xsl:transform version="1.0" xmlns="http://www.w3.org/2005/Atom" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:extension="http://exslt.org/common" xmlns:str="http://exslt.org/strings" xmlns:uri="http://www.w3.org/2000/07/uri43/uri.xsl?template=" xmlns:datetime="http://suda.co.uk/projects/microformats/datetime.xsl?template=" xmlns:h2a="http://rbach.priv.at/2007/hatom2atom" extension-element-prefixes="extension str"
    ...
    <xsl:template name="h2a:parse">
        ...
        <!--
            Enable users to use 'h2a:parse' as part of bigger stylesheet, for example: <xsl:import href="hAtom2Atom.xsl" /> [...] <xsl:for-each select="document('document-a.htm')"> <xsl:call-template name="h2a:parse"> <xsl:with-param name="source-uri">http://example.com/foo#abc</xsl:with-param> <xsl:with-param name="sanitize-html" select="0" /> </xsl:call-template> </xsl:for-each> [...] <xsl:for-each select="document('document-b.htm')"> <xsl:call-template name="h2a:parse"> <xsl:with-param
            name="source-uri">http://example.org/bar/foo#abcd</xsl:with-param> <xsl:with-param name="sanitize-html" select="1" /> </xsl:call-template> </xsl:for-each> So we can not use the values of the stylesheet's parameters directly, rather we have to pass on all the parameters recived by h2a:convert to all the templates. The usual method to do this would be: <xsl:template name="h2a:foo"> <xsl:param name="source-uri"/> <!- xsl:param for 'content-type' ,'debug-comments', 'sanitize-html', etc.
            -> [...] <xsl:call-template name="h2a:bar"> <xsl:with-param name="source-uri" select="$source-uri" /> <!- xsl:with-param for 'content-type' ,'debug-comments', 'sanitize-html', etc. -> </xsl:call-template> <xsl:apply-template name="h2a:bar"> <xsl:with-param name="source-uri" select="$source-uri" /> <!- xsl:with-param for 'content-type' ,'debug-comments', 'sanitize-html', etc. -> </xsl:apply-template> </xsl:template> As this would make maintance very complicated (for example: if we
            added a new parameter) , we store all parameters in a single data structure called "params". This simplifies the example above to: <xsl:template name="h2a:foo"> <xsl:param name="params"/> <xsl:call-template name="h2a:bar"> <xsl:with-param name="params" select="$params" /> </xsl:call-template> <xsl:apply-template name="h2a:bar"> <xsl:with-param name="params" select="$params" /> </xsl:apply-template> </xsl:template>
        -->
		...
		-->
            ...
            <h2a:implicit-feed>
                <xsl:value-of select="$implicit-feed" />
            </h2a:implicit-feed>
            <h2a:debug-comments>
                <xsl:value-of select="$debug-comments" />
            </h2a:debug-comments>
            <h2a:sanitize-html>
                <xsl:value-of select="$sanitize-html" />
            </h2a:sanitize-html>
            <h2a:tag-schemes>
                <xsl:value-of select="$tag-schemes" />
            </h2a:tag-schemes>
	...
	-->
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:message terminate="yes">WARN: No hAtom feeds and hAtom entries were found.</xsl:message>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template name="h2a:feed">
        ...
        <feed>
            ...
            <!--[extension]-->
            <xsl:apply-templates mode="h2a:add-feed-key-attribute" select=".">
                <xsl:with-param name="params" select="$params" />
		...
		<!-- Extract feed alternate link and put empty id -->
		<!--[extension]-->
    ...
    <xsl:template name="h2a:entry">
        ...
        <entry>
			<!--[extension]-->            
            <xsl:apply-templates mode="h2a:add-entry-key-attribute" select=".">
                <xsl:with-param name="params" select="$params"/>
                        ...
                        <xsl:for-each select="extension:node-set($entryLevelElements)/descendant::xhtml:a[contains(concat(' ',normalize-space(translate(@rel,'BOKMAR','bokmar')),' '),' bookmark ')][1]">
                            ...
                            <xsl:if test="not(@type)">
                                <xsl:attribute name="type">
                                    <xsl:value-of select="$params/h2a:content-type" />
                                </xsl:attribute>
                            </xsl:if>
                        </xsl:for-each>
                    </link>
...
<!--
 Walk the tree downwards to find
 a suitable entry-key.
-->
    <xsl:template mode="h2a:add-entry-key-attribute" match="xhtml:*">
        <xsl:param name="params"/>
        <xsl:param name="end" />
        <xsl:choose>
            <xsl:when test="@class[. = 'entry-key']">
            ...
            <xsl:otherwise>
                <xsl:if test="not(contains(concat(' ',normalize-space(@class),' '), concat(' ',$end,' ')))">
                    <xsl:apply-templates mode="h2a:add-entry-key-attribute" select="child::*">
                        <xsl:with-param name="params" select="$params"/>
...
<!--
 ...
 a suitable entry-key.
-->
    <xsl:template mode="h2a:add-feed-key-attribute" match="xhtml:*">
        <xsl:param name="params"/>
        <xsl:param name="end" />
        <xsl:choose>
            <xsl:when test="@class[. = 'feed-key']">
                <xsl:attribute name="key">
                    <xsl:value-of select="lower-case(@title)" />
                </xsl:attribute>
            </xsl:when>
            <xsl:otherwise>
                <xsl:if test="not(contains(concat(' ',normalize-space(@class),' '), concat(' ',$end,' ')))">
                    <xsl:apply-templates mode="h2a:add-feed-key-attribute" select="child::*">
                        <xsl:with-param name="params" select="$params"/>
                        <xsl:with-param name="end" select="$end" />
                    </xsl:apply-templates>
                </xsl:if>
